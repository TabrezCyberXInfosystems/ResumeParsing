...
function splitSectionsSmartly($lines) {
  ...
  foreach ($lines as $line) {
        ...
        // Contact info (phone and email in one line)
        $foundContact = false;
        if (preg_match_all('/\+?\d[\d\s\-\(\)]{7,}\d/', $line, $matches)) {
          ...
          foreach ($matches[0] as $match) {
              ...
              // Reject patterns that look like date ranges or education year ranges
              if (
                  $digitCount >= 10 && $digitCount <= 15 &&
      
                  // Not a full year range like 2016-2020
                  !preg_match('/\b\d{4}[-\/]\d{4}\b/', $cleaned) &&
      
                  // Not a full date like 13-07-1998
                  !preg_match('/\b\d{2}[-\/]\d{2}[-\/]\d{4}\b/', $cleaned) &&
      
                  // Not just a number like "70"
                  !preg_match('/^\d{2,3}$/', $cleaned) &&
      
                  // Not a mix like "2016-2020 70"
